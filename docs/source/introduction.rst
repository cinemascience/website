...
Cinema writers are available through common visualization applications.  A Cinema database export wizard leveraging the :ref:`label_cinemasci` is accessible interactively in `ParaView`_.  `VisIt`_ has Cinema export capabilities.  Or a Cinema database can be produced through ParaView's Catalyst or VisIt's LibSim in situ libraries.  The `Ascent`_ infrastructure can also be used to export Cinema databases.  A Cinema python library, cinema_lib, a command line tool is also available.  Lastly,  application-specific writers to create Cinema databases are also straightforward to implement in Python or other scripting languages.

The bounds of data exploration with Cinema are limited only by one's imagination.  Details of the Cinema database specifications are in :ref:`label_specifications` and basic viewers are described in :ref:`label_viewers`.
